...
| **App** | [A desktop and mobile shell](#the-app) — a real file picker, the system camera, and scanned links that open it |
...
`.dmg` and Windows SmartScreen will warn about the installer. Those warnings are
correct — nobody has vouched for those binaries — and the release notes carry the
incantation for each rather than pretending otherwise. Every file ships with a
