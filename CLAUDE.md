...
**The APK filenames are derived from `SHA256SUMS`, never spelled in the `fdroid` job.** That
file is generated by `sha256sum *` over the very directory that was uploaded, so it cannot
disagree with the Release by construction; a literal filename would be the one string a
naming change could break with nothing in the diff to read. The extraction tolerates a
leading `./` because `app-v0.1.0` shipped that shape and the bare-names change landed in
`f4bf3d2` after it — and it requires **exactly one** match per ABI, because picking the
first of two would be a coin toss resolved in favour of whatever `sha256sum` happened to
sort first. The names themselves are made in the `android` job, from Tauri's Gradle
flavours (`arm64`) to Android's ABI names (`arm64-v8a`), and that is the one place they are
spelled.

**Every Android APK of one version has its own `versionCode`, and the ranks are ordered.**
`build.gradle.kts` sets each flavour's to the base times ten plus a rank: universal 0,
arm 1, arm64 2, x86 3, x86_64 4. F-Droid requires a distinct code per APK, and its client
installs the highest one a device can run — an arm64 phone can usually run the armv7 APK
too, so arm64 must outrank it, and universal is lowest so that an install from the Release
page moves onto its own ABI's APK at the next update. Tauri's flavours carry no codes of
their own, so without that block all four APKs share one. The `android` job asserts every
code with `aapt2`, because the block is a hand-edit in `gen/` and losing it fails nowhere
else.
...
| `../../build.gradle.kts` (that is, `gen/android/app/`) | Three things. The version read from `src-tauri/Cargo.toml`: without it every release is versionCode 1 and none installs over another. The release signing config: without it a release APK is unsigned. The per-ABI `versionCode` block: without it the split APKs share one code and F-Droid cannot tell them apart. |
...
**The `android` job ships an APK per ABI and a universal one, from two invocations and not a
matrix.** Each ABI's native library is 8–12 MB stored uncompressed, so the universal APK is
45 MB where an arm64 phone needs about 14. 32-bit x86 is dropped from both, being emulators
only. A matrix of one job per ABI buys no wall-clock time, because the universal APK still
needs all three libraries in one Gradle project; the `--split-per-abi` invocation after it
reuses cargo's outputs. The `fdroid` job indexes the three per-ABI APKs and not the universal
one.
...
client that wants to reinstall it. The job then prunes to the newest three **versions**,
which is a size decision about one branch rather than a correctness one — versions and not
files, since a release is three APKs and pruning by file would keep one release and drop two
of its ABIs.
